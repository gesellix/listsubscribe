...
?>
  ...
  <!--
    ...
    function validateAddress(incoming)
    {
      ...
      if (emailstring.length < 6) {return false};
      ...
      if (beforeAmp.length = 0) {return false};
...
switch($_POST['sendoption'])
{
  case "send":
    ...
    switch ($maction)
    {
      case "add": $text = "subscribe ".$defaultpass." address=$email".$nl.$nl;
      break;
      case "del": $text = "unsubscribe ".$defaultpass." address=$email".$nl.$nl;
      break;
    }

    if ($text == "")
    {
      echo "<script> alert('Error encountered, please try again after reloading the site'); window.history.back(); </script>\n";
      exit;
    }
...
<?php
    ...
    else
    {
      switch ($maction)
      {
        case "add": Subscribe($email, $mailmanurl, $mailmanadminpass);
?>
      <center>
...
<?php
        break;
        case "del": unSubscribe($email, $mailmanurl, $mailmanadminpass);
?>
      <center>
